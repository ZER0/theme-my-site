...
exports.ZipWriter = function (zipPath) {
  ...
  this.addStream = function add(pathInZip, stream) {
    try {
      zw.addEntryStream(pathInZip, 0, Ci.nsIZipWriter.COMPRESSION_DEFAULT, stream, false);
    } catch(e) {
      console.log(e);
      console.log(pathInZip);
    }
  ...
  this.remove = function remove(entry) {
    try {
...
exports.ZipReader = function (zipPath) {
  ...
  this.read = function read(pattern) {
    let it = zr.findEntries(pattern);
    ...
    let entry = it.getNext();
    let stream = zr.getInputStream(entry);
    let count = stream.available();

    return NetUtil.readInputStreamToString(stream, count, {});
  }
